...
//
//       Instance Name:              sp_hde_32768_m32
//       Words:                      32768
//       Bits:                       32
//       Mux:                        32
//       Drive:                      6
...
//       
//       Creation Date:  Sat Jan 18 18:07:16 2025
//       Version: 	r0p0
//
...
module datapath_latch_sp_hde_32768_m32 (CLK,Q_update,D_update,SE,SI,D,DFTRAMBYP,mem_path,XQ,Q);
...
endmodule // datapath_latch_sp_hde_32768_m32
...
`ifdef POWER_PINS
module sp_hde_32768_m32 (VDDCE, VDDPE, VSSE, CENY, WENY, AY, GWENY, Q, SO, CLK, CEN,
    WEN, A, D, EMA, EMAW, TEN, TCEN, TWEN, TA, TD, GWEN, TGWEN, RET1N, SI, SE, DFTRAMBYP);
`else
module sp_hde_32768_m32 (CENY, WENY, AY, GWENY, Q, SO, CLK, CEN, WEN, A, D, EMA, EMAW,
...
`endif
 ...
 wire  CLK_;
  ...
  assign `ARM_UD_DP AY_ = (RET1N_ | pre_charge_st) ? ({15{DFTRAMBYP_}} & (TEN_ ? A_ : TA_)) : {15{1'bx}};
  ...
  begin
    if (GWEN_int !== 1'b1 && DFTRAMBYP_int=== 1'b0 && SE_int === 1'bx) begin
      ...
      if (GWEN_int !== 1'b1 || DFTRAMBYP_int === 1'b1 || DFTRAMBYP_int === 1'bx) begin
        row_mask =  ( {31'b0000000000000000000000000000000, writeEnable[31], 31'b0000000000000000000000000000000, writeEnable[30],
          31'b0000000000000000000000000000000, writeEnable[29], 31'b0000000000000000000000000000000, writeEnable[28],
          31'b0000000000000000000000000000000, writeEnable[27], 31'b0000000000000000000000000000000, writeEnable[26],
          31'b0000000000000000000000000000000, writeEnable[25], 31'b0000000000000000000000000000000, writeEnable[24],
          31'b0000000000000000000000000000000, writeEnable[23], 31'b0000000000000000000000000000000, writeEnable[22],
          31'b0000000000000000000000000000000, writeEnable[21], 31'b0000000000000000000000000000000, writeEnable[20],
          31'b0000000000000000000000000000000, writeEnable[19], 31'b0000000000000000000000000000000, writeEnable[18],
          31'b0000000000000000000000000000000, writeEnable[17], 31'b0000000000000000000000000000000, writeEnable[16],
          31'b0000000000000000000000000000000, writeEnable[15], 31'b0000000000000000000000000000000, writeEnable[14],
          31'b0000000000000000000000000000000, writeEnable[13], 31'b0000000000000000000000000000000, writeEnable[12],
          31'b0000000000000000000000000000000, writeEnable[11], 31'b0000000000000000000000000000000, writeEnable[10],
          31'b0000000000000000000000000000000, writeEnable[9], 31'b0000000000000000000000000000000, writeEnable[8],
          31'b0000000000000000000000000000000, writeEnable[7], 31'b0000000000000000000000000000000, writeEnable[6],
        ...
        new_data =  ( {31'b0000000000000000000000000000000, D_int[31], 31'b0000000000000000000000000000000, D_int[30],
          31'b0000000000000000000000000000000, D_int[29], 31'b0000000000000000000000000000000, D_int[28],
          31'b0000000000000000000000000000000, D_int[27], 31'b0000000000000000000000000000000, D_int[26],
          31'b0000000000000000000000000000000, D_int[25], 31'b0000000000000000000000000000000, D_int[24],
          31'b0000000000000000000000000000000, D_int[23], 31'b0000000000000000000000000000000, D_int[22],
          31'b0000000000000000000000000000000, D_int[21], 31'b0000000000000000000000000000000, D_int[20],
          31'b0000000000000000000000000000000, D_int[19], 31'b0000000000000000000000000000000, D_int[18],
          31'b0000000000000000000000000000000, D_int[17], 31'b0000000000000000000000000000000, D_int[16],
          31'b0000000000000000000000000000000, D_int[15], 31'b0000000000000000000000000000000, D_int[14],
          31'b0000000000000000000000000000000, D_int[13], 31'b0000000000000000000000000000000, D_int[12],
          31'b0000000000000000000000000000000, D_int[11], 31'b0000000000000000000000000000000, D_int[10],
          31'b0000000000000000000000000000000, D_int[9], 31'b0000000000000000000000000000000, D_int[8],
          31'b0000000000000000000000000000000, D_int[7], 31'b0000000000000000000000000000000, D_int[6],
        ...
        shifted_readLatch0 = (readLatch0 >> A_int[4:3]);
        mem_path = {shifted_readLatch0[124], shifted_readLatch0[120], shifted_readLatch0[116],
...
`endif
    ...
    if ((CLK_ === 1'bx || CLK_ === 1'bz) && RET1N_ !== 1'b0) begin
      ...
      if (DFTRAMBYP_=== 1'b1 || CEN_int != 1'b1) begin
        ...
        if (GWEN_int === 1'b1 || DFTRAMBYP_ == 1'b1) begin
          read_mux_sel0 = (TEN_ ? A_[4:3] : TA_[4:3] );
  ...
  datapath_latch_sp_hde_32768_m32 uDQ0 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(SI_int[0]), .D(D_int_bmux[0]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[0]), .XQ(XQ), .Q(Q_int[0]));
  datapath_latch_sp_hde_32768_m32 uDQ1 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[0]), .D(D_int_bmux[1]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[1]), .XQ(XQ), .Q(Q_int[1]));
  datapath_latch_sp_hde_32768_m32 uDQ2 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[1]), .D(D_int_bmux[2]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[2]), .XQ(XQ), .Q(Q_int[2]));
  datapath_latch_sp_hde_32768_m32 uDQ3 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[2]), .D(D_int_bmux[3]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[3]), .XQ(XQ), .Q(Q_int[3]));
  datapath_latch_sp_hde_32768_m32 uDQ4 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[3]), .D(D_int_bmux[4]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[4]), .XQ(XQ), .Q(Q_int[4]));
  datapath_latch_sp_hde_32768_m32 uDQ5 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[4]), .D(D_int_bmux[5]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[5]), .XQ(XQ), .Q(Q_int[5]));
  datapath_latch_sp_hde_32768_m32 uDQ6 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[5]), .D(D_int_bmux[6]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[6]), .XQ(XQ), .Q(Q_int[6]));
  datapath_latch_sp_hde_32768_m32 uDQ7 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[6]), .D(D_int_bmux[7]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[7]), .XQ(XQ), .Q(Q_int[7]));
  datapath_latch_sp_hde_32768_m32 uDQ8 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[7]), .D(D_int_bmux[8]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[8]), .XQ(XQ), .Q(Q_int[8]));
  datapath_latch_sp_hde_32768_m32 uDQ9 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[8]), .D(D_int_bmux[9]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[9]), .XQ(XQ), .Q(Q_int[9]));
  datapath_latch_sp_hde_32768_m32 uDQ10 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[9]), .D(D_int_bmux[10]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[10]), .XQ(XQ), .Q(Q_int[10]));
  datapath_latch_sp_hde_32768_m32 uDQ11 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[10]), .D(D_int_bmux[11]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[11]), .XQ(XQ), .Q(Q_int[11]));
  datapath_latch_sp_hde_32768_m32 uDQ12 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[11]), .D(D_int_bmux[12]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[12]), .XQ(XQ), .Q(Q_int[12]));
  datapath_latch_sp_hde_32768_m32 uDQ13 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[12]), .D(D_int_bmux[13]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[13]), .XQ(XQ), .Q(Q_int[13]));
  datapath_latch_sp_hde_32768_m32 uDQ14 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[13]), .D(D_int_bmux[14]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[14]), .XQ(XQ), .Q(Q_int[14]));
  datapath_latch_sp_hde_32768_m32 uDQ15 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[14]), .D(D_int_bmux[15]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[15]), .XQ(XQ), .Q(Q_int[15]));
  datapath_latch_sp_hde_32768_m32 uDQ16 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[17]), .D(D_int_bmux[16]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[16]), .XQ(XQ), .Q(Q_int[16]));
  datapath_latch_sp_hde_32768_m32 uDQ17 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[18]), .D(D_int_bmux[17]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[17]), .XQ(XQ), .Q(Q_int[17]));
  datapath_latch_sp_hde_32768_m32 uDQ18 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[19]), .D(D_int_bmux[18]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[18]), .XQ(XQ), .Q(Q_int[18]));
  datapath_latch_sp_hde_32768_m32 uDQ19 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[20]), .D(D_int_bmux[19]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[19]), .XQ(XQ), .Q(Q_int[19]));
  datapath_latch_sp_hde_32768_m32 uDQ20 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[21]), .D(D_int_bmux[20]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[20]), .XQ(XQ), .Q(Q_int[20]));
  datapath_latch_sp_hde_32768_m32 uDQ21 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[22]), .D(D_int_bmux[21]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[21]), .XQ(XQ), .Q(Q_int[21]));
  datapath_latch_sp_hde_32768_m32 uDQ22 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[23]), .D(D_int_bmux[22]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[22]), .XQ(XQ), .Q(Q_int[22]));
  datapath_latch_sp_hde_32768_m32 uDQ23 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[24]), .D(D_int_bmux[23]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[23]), .XQ(XQ), .Q(Q_int[23]));
  datapath_latch_sp_hde_32768_m32 uDQ24 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[25]), .D(D_int_bmux[24]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[24]), .XQ(XQ), .Q(Q_int[24]));
  datapath_latch_sp_hde_32768_m32 uDQ25 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[26]), .D(D_int_bmux[25]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[25]), .XQ(XQ), .Q(Q_int[25]));
  datapath_latch_sp_hde_32768_m32 uDQ26 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[27]), .D(D_int_bmux[26]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[26]), .XQ(XQ), .Q(Q_int[26]));
  datapath_latch_sp_hde_32768_m32 uDQ27 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[28]), .D(D_int_bmux[27]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[27]), .XQ(XQ), .Q(Q_int[27]));
  datapath_latch_sp_hde_32768_m32 uDQ28 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[29]), .D(D_int_bmux[28]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[28]), .XQ(XQ), .Q(Q_int[28]));
  datapath_latch_sp_hde_32768_m32 uDQ29 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[30]), .D(D_int_bmux[29]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[29]), .XQ(XQ), .Q(Q_int[29]));
  datapath_latch_sp_hde_32768_m32 uDQ30 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(Q_int[31]), .D(D_int_bmux[30]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[30]), .XQ(XQ), .Q(Q_int[30]));
  datapath_latch_sp_hde_32768_m32 uDQ31 (.CLK(CLK), .Q_update(Q_update), .D_update(D_sh_update), .SE(SE_), .SI(SI_int[1]), .D(D_int_bmux[31]), .DFTRAMBYP(DFTRAMBYP_), .mem_path(mem_path[31]), .XQ(XQ), .Q(Q_int[31]));
...
`endif
  ...
  reg NOT_WEN2, NOT_WEN1, NOT_WEN0, NOT_A14, NOT_A13, NOT_A12, NOT_A11, NOT_A10, NOT_A9;
  reg NOT_A8, NOT_A7, NOT_A6, NOT_A5, NOT_A4, NOT_A3, NOT_A2, NOT_A1, NOT_A0, NOT_D31;
  ...
  reg NOT_TWEN4, NOT_TWEN3, NOT_TWEN2, NOT_TWEN1, NOT_TWEN0, NOT_TA14, NOT_TA13, NOT_TA12;
  reg NOT_TA11, NOT_TA10, NOT_TA9, NOT_TA8, NOT_TA7, NOT_TA6, NOT_TA5, NOT_TA4, NOT_TA3;
  reg NOT_TA2, NOT_TA1, NOT_TA0, NOT_TD31, NOT_TD30, NOT_TD29, NOT_TD28, NOT_TD27;
 ...
 wire  CLK_;
  ...
  buf B47(AY[14], AY_[14]);
  buf B48(GWENY, GWENY_);
  buf B49(Q[0], Q_[0]);
  buf B50(Q[1], Q_[1]);
  buf B51(Q[2], Q_[2]);
  buf B52(Q[3], Q_[3]);
  ...
  buf B83(CLK_, CLK);
  buf B84(CEN_, CEN);
  buf B85(WEN_[0], WEN[0]);
  buf B86(WEN_[1], WEN[1]);
  buf B87(WEN_[2], WEN[2]);
  buf B88(WEN_[3], WEN[3]);
  buf B89(WEN_[4], WEN[4]);
  buf B90(WEN_[5], WEN[5]);
  buf B91(WEN_[6], WEN[6]);
  buf B92(WEN_[7], WEN[7]);
  buf B93(WEN_[8], WEN[8]);
  buf B94(WEN_[9], WEN[9]);
  buf B95(WEN_[10], WEN[10]);
  buf B96(WEN_[11], WEN[11]);
  buf B97(WEN_[12], WEN[12]);
  buf B98(WEN_[13], WEN[13]);
  buf B99(WEN_[14], WEN[14]);
  buf B100(WEN_[15], WEN[15]);
  buf B101(WEN_[16], WEN[16]);
  buf B102(WEN_[17], WEN[17]);
  buf B103(WEN_[18], WEN[18]);
  buf B104(WEN_[19], WEN[19]);
  buf B105(WEN_[20], WEN[20]);
  buf B106(WEN_[21], WEN[21]);
  buf B107(WEN_[22], WEN[22]);
  buf B108(WEN_[23], WEN[23]);
  buf B109(WEN_[24], WEN[24]);
  buf B110(WEN_[25], WEN[25]);
  buf B111(WEN_[26], WEN[26]);
  buf B112(WEN_[27], WEN[27]);
  buf B113(WEN_[28], WEN[28]);
  buf B114(WEN_[29], WEN[29]);
  buf B115(WEN_[30], WEN[30]);
  buf B116(WEN_[31], WEN[31]);
  buf B117(A_[0], A[0]);
  buf B118(A_[1], A[1]);
  buf B119(A_[2], A[2]);
  buf B120(A_[3], A[3]);
  ...
  buf B162(D_[30], D[30]);
  buf B163(D_[31], D[31]);
  buf B164(EMA_[0], EMA[0]);
  buf B165(EMA_[1], EMA[1]);
  buf B166(EMA_[2], EMA[2]);
  buf B167(EMAW_[0], EMAW[0]);
  buf B168(EMAW_[1], EMAW[1]);
  buf B169(TEN_, TEN);
  buf B170(TCEN_, TCEN);
  buf B171(TWEN_[0], TWEN[0]);
  buf B172(TWEN_[1], TWEN[1]);
  buf B173(TWEN_[2], TWEN[2]);
  buf B174(TWEN_[3], TWEN[3]);
  buf B175(TWEN_[4], TWEN[4]);
  buf B176(TWEN_[5], TWEN[5]);
  buf B177(TWEN_[6], TWEN[6]);
  buf B178(TWEN_[7], TWEN[7]);
  buf B179(TWEN_[8], TWEN[8]);
  buf B180(TWEN_[9], TWEN[9]);
  buf B181(TWEN_[10], TWEN[10]);
  buf B182(TWEN_[11], TWEN[11]);
  buf B183(TWEN_[12], TWEN[12]);
  buf B184(TWEN_[13], TWEN[13]);
  buf B185(TWEN_[14], TWEN[14]);
  buf B186(TWEN_[15], TWEN[15]);
  buf B187(TWEN_[16], TWEN[16]);
  buf B188(TWEN_[17], TWEN[17]);
  buf B189(TWEN_[18], TWEN[18]);
  buf B190(TWEN_[19], TWEN[19]);
  buf B191(TWEN_[20], TWEN[20]);
  buf B192(TWEN_[21], TWEN[21]);
  buf B193(TWEN_[22], TWEN[22]);
  buf B194(TWEN_[23], TWEN[23]);
  buf B195(TWEN_[24], TWEN[24]);
  buf B196(TWEN_[25], TWEN[25]);
  buf B197(TWEN_[26], TWEN[26]);
  buf B198(TWEN_[27], TWEN[27]);
  buf B199(TWEN_[28], TWEN[28]);
  buf B200(TWEN_[29], TWEN[29]);
  buf B201(TWEN_[30], TWEN[30]);
  buf B202(TWEN_[31], TWEN[31]);
  buf B203(TA_[0], TA[0]);
  buf B204(TA_[1], TA[1]);
  buf B205(TA_[2], TA[2]);
  buf B206(TA_[3], TA[3]);
  ...
  buf B211(TA_[8], TA[8]);
  buf B212(TA_[9], TA[9]);
  buf B213(TA_[10], TA[10]);
  buf B214(TA_[11], TA[11]);
  buf B215(TA_[12], TA[12]);
  buf B216(TA_[13], TA[13]);
  buf B217(TA_[14], TA[14]);
  buf B218(TD_[0], TD[0]);
  buf B219(TD_[1], TD[1]);
  buf B220(TD_[2], TD[2]);
  buf B221(TD_[3], TD[3]);
  ...
  buf B226(TD_[8], TD[8]);
  buf B227(TD_[9], TD[9]);
  buf B228(TD_[10], TD[10]);
  buf B229(TD_[11], TD[11]);
  buf B230(TD_[12], TD[12]);
  buf B231(TD_[13], TD[13]);
  buf B232(TD_[14], TD[14]);
  buf B233(TD_[15], TD[15]);
  buf B234(TD_[16], TD[16]);
  buf B235(TD_[17], TD[17]);
  buf B236(TD_[18], TD[18]);
  buf B237(TD_[19], TD[19]);
  buf B238(TD_[20], TD[20]);
  buf B239(TD_[21], TD[21]);
  buf B240(TD_[22], TD[22]);
  buf B241(TD_[23], TD[23]);
  buf B242(TD_[24], TD[24]);
  buf B243(TD_[25], TD[25]);
  buf B244(TD_[26], TD[26]);
  buf B245(TD_[27], TD[27]);
  buf B246(TD_[28], TD[28]);
  buf B247(TD_[29], TD[29]);
  buf B248(TD_[30], TD[30]);
  buf B249(TD_[31], TD[31]);
  buf B250(GWEN_, GWEN);
  buf B251(TGWEN_, TGWEN);
  buf B252(RET1N_, RET1N);
  buf B253(SI_[0], SI[0]);
  buf B254(SI_[1], SI[1]);
  buf B255(SE_, SE);
  buf B256(DFTRAMBYP_, DFTRAMBYP);
  ...
  assign AY_ = (RET1N_ | pre_charge_st) ? ({15{DFTRAMBYP_}} & (TEN_ ? A_ : TA_)) : {15{1'bx}};
...
`endif
  ...
  specify
    ...
    if (DFTRAMBYP == 1'b1 && A[14] == 1'b0 && TA[14] == 1'b1)
       (TEN -=> AY[14]) = (`ARM_MEM_PROP, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP);
    if (DFTRAMBYP == 1'b1 && A[14] == 1'b1 && TA[14] == 1'b0)
       (TEN +=> AY[14]) = (`ARM_MEM_PROP, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP);
    ...
    if (DFTRAMBYP == 1'b1 && A[0] == 1'b1 && TA[0] == 1'b0)
       (TEN +=> AY[0]) = (`ARM_MEM_PROP, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP);
    if (DFTRAMBYP == 1'b1 && TEN == 1'b1)
       (A[14] +=> AY[14]) = (`ARM_MEM_PROP, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP);
    if (DFTRAMBYP == 1'b1 && TEN == 1'b1)
    ...
    if (DFTRAMBYP == 1'b1 && TEN == 1'b0)
       (TA[14] +=> AY[14]) = (`ARM_MEM_PROP, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP);
    if (DFTRAMBYP == 1'b1 && TEN == 1'b0)
    ...
    if (RET1N == 1'b1)
       (DFTRAMBYP +=> AY[14]) = (`ARM_MEM_PROP, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP, `ARM_MEM_RETAIN, `ARM_MEM_PROP);
    if (RET1N == 1'b1)
   ...
   `endif
    ...
    $setuphold(posedge CLK &&& RET1Neq1aTENeq1aCENeq0, posedge A[14], `ARM_MEM_SETUP, `ARM_MEM_HOLD, NOT_A14);
    ...
    $setuphold(posedge CLK &&& RET1Neq1aTENeq1aCENeq0, negedge A[14], `ARM_MEM_SETUP, `ARM_MEM_HOLD, NOT_A14);
    ...
    $setuphold(posedge CLK &&& RET1Neq1aTENeq0aTCENeq0, posedge TA[14], `ARM_MEM_SETUP, `ARM_MEM_HOLD, NOT_TA14);
    ...
    $setuphold(posedge CLK &&& RET1Neq1aTENeq0aTCENeq0, negedge TA[14], `ARM_MEM_SETUP, `ARM_MEM_HOLD, NOT_TA14);
    ...
    if (GWEN_int !== 1'b1 && DFTRAMBYP_int=== 1'b0 && SE_int === 1'bx) begin
      ...
      if (GWEN_int !== 1'b1 || DFTRAMBYP_int === 1'b1 || DFTRAMBYP_int === 1'bx) begin
        ...
        readLatch0 = {data_out[1016], data_out[1008], data_out[1000], data_out[992],
          ...
          data_out[8], data_out[0]};
        shifted_readLatch0 = (readLatch0 >> A_int[4:3]);
        Q_int = {shifted_readLatch0[124], shifted_readLatch0[120], shifted_readLatch0[116],
          shifted_readLatch0[112], shifted_readLatch0[108], shifted_readLatch0[104],
          shifted_readLatch0[100], shifted_readLatch0[96], shifted_readLatch0[92],
          shifted_readLatch0[88], shifted_readLatch0[84], shifted_readLatch0[80], shifted_readLatch0[76],
          shifted_readLatch0[72], shifted_readLatch0[68], shifted_readLatch0[64], shifted_readLatch0[60],
...
`endif
 ...
 wire  CLK_;
  ...
  buf B257(CENY, CENY_);
  buf B258(WENY[0], WENY_[0]);
  buf B259(WENY[1], WENY_[1]);
  buf B260(WENY[2], WENY_[2]);
  buf B261(WENY[3], WENY_[3]);
  buf B262(WENY[4], WENY_[4]);
  buf B263(WENY[5], WENY_[5]);
  buf B264(WENY[6], WENY_[6]);
  buf B265(WENY[7], WENY_[7]);
  buf B266(WENY[8], WENY_[8]);
  buf B267(WENY[9], WENY_[9]);
  buf B268(WENY[10], WENY_[10]);
  buf B269(WENY[11], WENY_[11]);
  buf B270(WENY[12], WENY_[12]);
  buf B271(WENY[13], WENY_[13]);
  buf B272(WENY[14], WENY_[14]);
  buf B273(WENY[15], WENY_[15]);
  buf B274(WENY[16], WENY_[16]);
  buf B275(WENY[17], WENY_[17]);
  buf B276(WENY[18], WENY_[18]);
  buf B277(WENY[19], WENY_[19]);
  buf B278(WENY[20], WENY_[20]);
  buf B279(WENY[21], WENY_[21]);
  buf B280(WENY[22], WENY_[22]);
  buf B281(WENY[23], WENY_[23]);
  buf B282(WENY[24], WENY_[24]);
  buf B283(WENY[25], WENY_[25]);
  buf B284(WENY[26], WENY_[26]);
  buf B285(WENY[27], WENY_[27]);
  buf B286(WENY[28], WENY_[28]);
  buf B287(WENY[29], WENY_[29]);
  buf B288(WENY[30], WENY_[30]);
  buf B289(WENY[31], WENY_[31]);
  buf B290(AY[0], AY_[0]);
  buf B291(AY[1], AY_[1]);
  buf B292(AY[2], AY_[2]);
  buf B293(AY[3], AY_[3]);
  ...
  buf B298(AY[8], AY_[8]);
  buf B299(AY[9], AY_[9]);
  buf B300(AY[10], AY_[10]);
  buf B301(AY[11], AY_[11]);
  buf B302(AY[12], AY_[12]);
  buf B303(AY[13], AY_[13]);
  buf B304(AY[14], AY_[14]);
  buf B305(GWENY, GWENY_);
  buf B306(Q[0], Q_[0]);
  buf B307(Q[1], Q_[1]);
  buf B308(Q[2], Q_[2]);
  buf B309(Q[3], Q_[3]);
  ...
  buf B340(CLK_, CLK);
  buf B341(CEN_, CEN);
  buf B342(WEN_[0], WEN[0]);
  buf B343(WEN_[1], WEN[1]);
  buf B344(WEN_[2], WEN[2]);
  buf B345(WEN_[3], WEN[3]);
  buf B346(WEN_[4], WEN[4]);
  buf B347(WEN_[5], WEN[5]);
  buf B348(WEN_[6], WEN[6]);
  buf B349(WEN_[7], WEN[7]);
  buf B350(WEN_[8], WEN[8]);
  buf B351(WEN_[9], WEN[9]);
  buf B352(WEN_[10], WEN[10]);
  buf B353(WEN_[11], WEN[11]);
  buf B354(WEN_[12], WEN[12]);
  buf B355(WEN_[13], WEN[13]);
  buf B356(WEN_[14], WEN[14]);
  buf B357(WEN_[15], WEN[15]);
  buf B358(WEN_[16], WEN[16]);
  buf B359(WEN_[17], WEN[17]);
  buf B360(WEN_[18], WEN[18]);
  buf B361(WEN_[19], WEN[19]);
  buf B362(WEN_[20], WEN[20]);
  buf B363(WEN_[21], WEN[21]);
  buf B364(WEN_[22], WEN[22]);
  buf B365(WEN_[23], WEN[23]);
  buf B366(WEN_[24], WEN[24]);
  buf B367(WEN_[25], WEN[25]);
  buf B368(WEN_[26], WEN[26]);
  buf B369(WEN_[27], WEN[27]);
  buf B370(WEN_[28], WEN[28]);
  buf B371(WEN_[29], WEN[29]);
  buf B372(WEN_[30], WEN[30]);
  buf B373(WEN_[31], WEN[31]);
  buf B374(A_[0], A[0]);
  buf B375(A_[1], A[1]);
  buf B376(A_[2], A[2]);
  buf B377(A_[3], A[3]);
  ...
  buf B419(D_[30], D[30]);
  buf B420(D_[31], D[31]);
  buf B421(EMA_[0], EMA[0]);
  buf B422(EMA_[1], EMA[1]);
  buf B423(EMA_[2], EMA[2]);
  buf B424(EMAW_[0], EMAW[0]);
  buf B425(EMAW_[1], EMAW[1]);
  buf B426(TEN_, TEN);
  buf B427(TCEN_, TCEN);
  buf B428(TWEN_[0], TWEN[0]);
  buf B429(TWEN_[1], TWEN[1]);
  buf B430(TWEN_[2], TWEN[2]);
  buf B431(TWEN_[3], TWEN[3]);
  buf B432(TWEN_[4], TWEN[4]);
  buf B433(TWEN_[5], TWEN[5]);
  buf B434(TWEN_[6], TWEN[6]);
  buf B435(TWEN_[7], TWEN[7]);
  buf B436(TWEN_[8], TWEN[8]);
  buf B437(TWEN_[9], TWEN[9]);
  buf B438(TWEN_[10], TWEN[10]);
  buf B439(TWEN_[11], TWEN[11]);
  buf B440(TWEN_[12], TWEN[12]);
  buf B441(TWEN_[13], TWEN[13]);
  buf B442(TWEN_[14], TWEN[14]);
  buf B443(TWEN_[15], TWEN[15]);
  buf B444(TWEN_[16], TWEN[16]);
  buf B445(TWEN_[17], TWEN[17]);
  buf B446(TWEN_[18], TWEN[18]);
  buf B447(TWEN_[19], TWEN[19]);
  buf B448(TWEN_[20], TWEN[20]);
  buf B449(TWEN_[21], TWEN[21]);
  buf B450(TWEN_[22], TWEN[22]);
  buf B451(TWEN_[23], TWEN[23]);
  buf B452(TWEN_[24], TWEN[24]);
  buf B453(TWEN_[25], TWEN[25]);
  buf B454(TWEN_[26], TWEN[26]);
  buf B455(TWEN_[27], TWEN[27]);
  buf B456(TWEN_[28], TWEN[28]);
  buf B457(TWEN_[29], TWEN[29]);
  buf B458(TWEN_[30], TWEN[30]);
  buf B459(TWEN_[31], TWEN[31]);
  buf B460(TA_[0], TA[0]);
  buf B461(TA_[1], TA[1]);
  buf B462(TA_[2], TA[2]);
  buf B463(TA_[3], TA[3]);
  ...
  buf B468(TA_[8], TA[8]);
  buf B469(TA_[9], TA[9]);
  buf B470(TA_[10], TA[10]);
  buf B471(TA_[11], TA[11]);
  buf B472(TA_[12], TA[12]);
  buf B473(TA_[13], TA[13]);
  buf B474(TA_[14], TA[14]);
  buf B475(TD_[0], TD[0]);
  buf B476(TD_[1], TD[1]);
  buf B477(TD_[2], TD[2]);
  buf B478(TD_[3], TD[3]);
  ...
  buf B483(TD_[8], TD[8]);
  buf B484(TD_[9], TD[9]);
  buf B485(TD_[10], TD[10]);
  buf B486(TD_[11], TD[11]);
  buf B487(TD_[12], TD[12]);
  buf B488(TD_[13], TD[13]);
  buf B489(TD_[14], TD[14]);
  buf B490(TD_[15], TD[15]);
  buf B491(TD_[16], TD[16]);
  buf B492(TD_[17], TD[17]);
  buf B493(TD_[18], TD[18]);
  buf B494(TD_[19], TD[19]);
  buf B495(TD_[20], TD[20]);
  buf B496(TD_[21], TD[21]);
  buf B497(TD_[22], TD[22]);
  buf B498(TD_[23], TD[23]);
  buf B499(TD_[24], TD[24]);
  buf B500(TD_[25], TD[25]);
  buf B501(TD_[26], TD[26]);
  buf B502(TD_[27], TD[27]);
  buf B503(TD_[28], TD[28]);
  buf B504(TD_[29], TD[29]);
  buf B505(TD_[30], TD[30]);
  buf B506(TD_[31], TD[31]);
  buf B507(GWEN_, GWEN);
  buf B508(TGWEN_, TGWEN);
  buf B509(RET1N_, RET1N);
  buf B510(SI_[0], SI[0]);
  buf B511(SI_[1], SI[1]);
  buf B512(SE_, SE);
  buf B513(DFTRAMBYP_, DFTRAMBYP);
  ...
  assign GWENY_ = (RET1N_ | pre_charge_st) ? (DFTRAMBYP_ & (TEN_ ? GWEN_ : TGWEN_)) : 1'bx;
   `ifdef ARM_FAULT_MODELING
     sp_hde_32768_m32_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .DFTRAMBYP(DFTRAMBYP_int), .SE(SE_int), .GWEN(GWEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
  assign Q_ = (RET1N_ | pre_charge_st) ? ((Q_int)) : {32{1'bx}};
  `endif
  assign SO_ = (RET1N_ | pre_charge_st) ? ({Q_[16], Q_[15]}) : {2{1'bx}};
...
`endif
`timescale 1ns/1ps
module sp_hde_32768_m32_error_injection (Q_out, Q_in, CLK, A, CEN, DFTRAMBYP, SE, WEN, GWEN);
   output [31:0] Q_out;
   input [31:0] Q_in;
   input CLK;
   input [14:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(15'd7295,5'd6,2'd1,2'd0);
   `endif
end
   task add_fault;
   //This task injects fault in memory
   //In order to inject fault in redundant column for Bit 0 to 15, column address
   //should have value in range of 24 to 31
...
begin
   ...
   begin
      ...
      begin
         ...
         begin
            if (row_address == A[14:5] && column_address == A[4:0])
            begin
               if (bitPlace < 16)
